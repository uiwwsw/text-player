...
import { animationStyles, resolveAnimationStyle, defaultAnimationForPunctuation } from "@/lib/animations";
...
export function ScriptPlayer({
  ...
  useEffect(() => {
    ...
    // Resolve animation style to get duration
    const animStyle = settings?.animationStyle ?? defaultAnimationForPunctuation[activeSlide.punctuation];
    const animDuration = animationStyles[animStyle].durationMs;

    // Total time = Animation time + Stay time (duration setting)
    const stayDuration = settings?.duration ?? activeSlide.baseDuration;
    const totalDuration = animDuration + stayDuration;
  ...
  const handleDurationChange = (delta: number) => {
    ...
    const currentStay = activeSettings.duration ?? activeSlide.baseDuration;
    const next = Math.max(0, currentStay + delta); // Allow 0 stay time if they want it fast
